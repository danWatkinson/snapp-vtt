...
export default function WorldSubTabs() {
  const {
    worlds,
    selectedIds,
    subTab,
  ...
  return (
    <section className="space-y-2">
      <h3 className="text-sm font-semibold snapp-heading">
        Views for {selectedWorldName ?? "selected world"}
      </h3>
      <TabList aria-label="World views" variant="planning">
        {(["World Entities", "Campaigns", "Story Arcs", "Users"] as const).map(
          (label) => {
            const isActive = subTab === label;
            return (
              <TabButton
                key={label}
                isActive={isActive}
