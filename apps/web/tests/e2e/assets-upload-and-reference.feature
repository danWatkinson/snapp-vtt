Feature: World builder uploads and references digital assets
  ...
  Scenario: World builder uploads an audio asset for use in a scene
    When the admin signs in to the system as "admin"
    And the admin navigates to the "Assets" library screen
    And the admin uploads an audio asset "forest-ambience.mp3"
  ...
  Scenario: World builder references an uploaded audio asset in a scene
    ...
    And the admin selects campaign "The Eldorian Saga"
    And the admin ensures session "Session 1" exists for campaign "The Eldorian Saga"
    And the admin ensures scene "Into the Woods" exists in session "Session 1"
    And the admin sets the ambience audio asset for scene "Into the Woods" to "forest-ambience.mp3"
